...
def model_builder(hp):
    model = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Dense(
                    units=hp.Int('num_of_neurons1', min_value=10, max_value=50, step=10),
                    activation=tf.keras.layers.LeakyReLU(alpha=0.01)
                ),
                tf.keras.layers.Dense(
                    units=hp.Int('num_of_neurons2', min_value=10, max_value=50, step=10), 
                    activation=tf.keras.layers.LeakyReLU(alpha=0.01)
                ),
    ...
    hp_learning_rate = hp.Choice('learning_rate', values=[1e-1, 1e-2, 5e-3, 1e-3]) 
